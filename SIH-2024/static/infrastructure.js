document.addEventListener("DOMContentLoaded", () => {
    ...
    demographicSelect.addEventListener("change", async () => {
        ...
        if (selectedDemographic) {
            const response = await fetch("/get_columns_infrastructure", {
                method: "POST",
    ...
    generateButton.addEventListener("click", async () => {
        ...
        const response = await fetch("/generate_result_infrastructure", {
            method: "POST",
        ...
        chart = new Chart(chartCanvas, {
            type: graph,
            data: {
                labels: result.data.map(row => row['District name']), // Labels based on Region or any column
                datasets: [{
                    label: column,
